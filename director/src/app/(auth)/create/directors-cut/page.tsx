  ...
  if (loading) {
    return (
      <main className="flex-grow flex items-center justify-center">
        <div className="text-[#adaaaa] text-sm">Loading brief...</div>
      </main>
    );
  }
      ...
      <section className="flex-grow space-y-12">
        ...
        <div className="space-y-4">
          ...
          <div className="w-full h-1 bg-[#262626] rounded-full overflow-hidden">
            <div className="w-[37.5%] h-full bg-[#ff9064] rounded-full shadow-[0_0_40px_rgba(255,144,100,0.1)]"></div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#131313] rounded-xl p-6 space-y-4 border border-[#494847]/10">
            <div className="space-y-1">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Title</label>
              <input
                type="text"
                value={brief?.storyline.title ?? ''}
                onChange={(e) => updateStoryline({ title: e.target.value })}
                placeholder="Working title for your project"
                className="w-full bg-[#0e0e0e] border border-[#494847]/20 rounded-lg px-4 py-3 text-white placeholder-[#555] focus:border-[#ff9064] focus:outline-none transition"
              />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Synopsis</label>
              <textarea
                rows={4}
                ...
                className="w-full bg-[#0e0e0e] border border-[#494847]/20 rounded-lg px-4 py-3 text-white placeholder-[#555] focus:border-[#ff9064] focus:outline-none transition resize-none"
              />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Theme</label>
              <input
                type="text"
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#131313] rounded-xl p-6 space-y-4 border border-[#494847]/10">
            <div className="space-y-1">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Mood</label>
              <input
                type="text"
            ...
            {/* Color Palette */}
            <div className="space-y-2">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Color Palette</label>
              <div className="flex flex-wrap gap-2">
                {brief?.visualStyle.colorPalette.map((color, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-2 bg-[#262626] rounded-full px-3 py-1.5 text-xs text-white"
                  >
                    <span
                    ...
                    <button
                      ...
                      className="text-[#adaaaa] hover:text-white transition"
              ...
              {(brief?.visualStyle.colorPalette.length ?? 0) < 8 && (
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="e.g., #ff9064 or warm gold"
                    className="flex-grow bg-[#0e0e0e] border border-[#494847]/20 rounded-lg px-4 py-2 text-white text-sm placeholder-[#555] focus:border-[#ff9064] focus:outline-none transition"
                  />
                  <button
                    onClick={handleAddColor}
                    className="px-4 py-2 rounded-lg border border-[#ff9064] text-[#ff9064] text-sm font-bold hover:bg-[#ff9064]/10 transition"
                  >
                    Add
                  </button>
                </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-label uppercase tracking-widest text-[#adaaaa]">Cinematography References</label>
              <div className="flex flex-wrap gap-2">
                {brief?.visualStyle.cinematographyRefs.map((ref, i) => (
                  <span
                    ...
                    className="inline-flex items-center gap-2 bg-[#262626] rounded-full px-3 py-1.5 text-xs text-white"
                  >
                    <span className="material-symbols-outlined text-[14px] text-[#ff9064]">movie</span>
                    {ref}
                    <button
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#131313] rounded-xl p-6 space-y-4 border border-[#494847]/10">
            {brief?.narrativeBeats.beats.map((beat, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-[#0e0e0e] border border-[#494847]/10">
                {/* Segment number */}
                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[#262626] flex items-center justify-center text-xs font-bold text-[#ff9064]">
                  {i + 1}
                </div>
                {/* Fields */}
                <div className="flex-grow grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div className="space-y-1">
                    <label className="text-[10px] font-label uppercase tracking-widest text-[#adaaaa]">Label</label>
                    <input
                      type="text"
                      value={beat.label}
                      onChange={(e) => updateBeat(i, { label: e.target.value })}
                      placeholder="e.g., Opening hook"
                      className="w-full bg-[#131313] border border-[#494847]/20 rounded-lg px-3 py-2 text-white text-sm placeholder-[#555] focus:border-[#ff9064] focus:outline-none transition"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-label uppercase tracking-widest text-[#adaaaa]">Emotion</label>
                    <input
                      type="text"
                      value={beat.emotion}
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-label uppercase tracking-widest text-[#adaaaa]">Transition</label>
                    <select
                      value={beat.transition}
                      onChange={(e) => updateBeat(i, { transition: e.target.value })}
                      className="w-full bg-[#131313] border border-[#494847]/20 rounded-lg px-3 py-2 text-white text-sm focus:border-[#ff9064] focus:outline-none transition appearance-none"
                    >
                ...
                <button
                  ...
                  className="flex-shrink-0 mt-4 text-[#adaaaa] hover:text-red-400 transition"
            ...
            {(brief?.narrativeBeats.beats.length ?? 0) < 12 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-[#ff9064] text-[#ff9064] text-sm font-bold hover:bg-[#ff9064]/10 transition"
              >
        ...
        <div className="sticky top-24 bg-[#131313]/50 backdrop-blur-md border border-[#494847]/10 rounded-2xl p-6 space-y-6">
            ...
            {brief?.storyline.title || brief?.storyline.theme ? (
              <div className="p-4 rounded-xl bg-[#262626]/40 border-l-2 border-[#ff9064]">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] uppercase tracking-wider text-[#adaaaa]">Story Foundation</span>
                </div>
                <p className="text-xs text-white leading-relaxed">
                  {brief.storyline.title && <><strong>{brief.storyline.title}</strong><br /></>}
                  {brief.storyline.theme && <span className="text-[#adaaaa]">Theme: {brief.storyline.theme}</span>}
                </p>
              </div>
            ) : (
              <div className="p-4 rounded-xl bg-[#000000] border border-dashed border-[#494847]/20 flex flex-col items-center justify-center text-center py-6">
                <span className="material-symbols-outlined text-[#adaaaa]/30 text-2xl mb-2">auto_stories</span>
                <p className="text-[10px] text-[#adaaaa] italic">Waiting for storyline...</p>
              </div>
            )}
            ...
            {brief?.visualStyle.mood || (brief?.visualStyle.colorPalette.length ?? 0) > 0 ? (
              <div className="p-4 rounded-xl bg-[#262626]/40 border-l-2 border-[#81e9ff]">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] uppercase tracking-wider text-[#adaaaa]">Visual Direction</span>
                </div>
                <div className="space-y-1">
              ...
              <div className="p-4 rounded-xl bg-[#000000] border border-dashed border-[#494847]/20 flex flex-col items-center justify-center text-center py-6">
                <span className="material-symbols-outlined text-[#adaaaa]/30 text-2xl mb-2">palette</span>
                <p className="text-[10px] text-[#adaaaa] italic">Waiting for visual style...</p>
              </div>
            )}

            {/* Beat Map */}
            {(brief?.narrativeBeats.beats.length ?? 0) > 0 ? (
              <div className="p-4 rounded-xl bg-[#262626]/40 border-l-2 border-[#c9ff81]">
                <div className="flex items-center gap-2 mb-1">
                  <span className="material-symbols-outlined text-xs text-[#c9ff81]" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                  <span className="text-[10px] uppercase tracking-wider text-[#adaaaa]">Beat Map</span>
                </div>
                <p className="text-xs text-white leading-relaxed">
              ...
              <div className="p-4 rounded-xl bg-[#000000] border border-dashed border-[#494847]/20 flex flex-col items-center justify-center text-center py-6">
                <span className="material-symbols-outlined text-[#adaaaa]/30 text-2xl mb-2">timeline</span>
                <p className="text-[10px] text-[#adaaaa] italic">Waiting for narrative beats...</p>
              </div>
            )}
          ...
          <div className="pt-4 border-t border-[#494847]/10">
            <div className="flex items-start gap-3 opacity-60">
              ...
              <p className="text-[10px] text-[#adaaaa] leading-tight">Your brief feeds into the Creative Director agent, which will compose a sequence plan from your narrative beats and visual preferences.</p>
            </div>
          </div>
        </div>
      </aside>
