  ...
  return (
    <div className="h-12 bg-surface-container-low/95 backdrop-blur-lg border-b border-white/5 flex items-center px-4 gap-3 relative">
      ...
      <button
        onClick={undo}
        disabled={!canUndo}
        className="flex items-center px-1.5 py-1.5 rounded-lg bg-white/5 text-on-surface/50 text-xs hover:bg-white/10 hover:text-on-surface/70 transition-colors disabled:opacity-20 focus:outline-none focus:ring-2 focus:ring-primary/30"
      ...
      <button
        ...
        className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs transition-colors disabled:opacity-30 focus:outline-none focus:ring-2 focus:ring-primary/30 ${
          saveFlash
            ...
            : 'bg-white/5 text-on-surface/50 hover:bg-white/10 hover:text-on-surface/70'
        }`}
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white/5 text-on-surface/50 text-xs hover:bg-white/10 hover:text-on-surface/70 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30"
        ...
        {showLoadMenu && (
          <div className="absolute top-full left-0 mt-1 w-64 bg-surface-container border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="px-3 pt-2.5 pb-1">
              <span className="text-[9px] font-medium text-on-surface/30 uppercase tracking-wider">Presets</span>
            </div>
            {PIPELINE_PRESETS.map((preset) => (
              ...
              >
                <div className="text-xs text-on-surface/70">{preset.name}</div>
                <div className="text-[10px] text-on-surface/30">{preset.description}</div>
              </button>
            ))}
            ...
            {savedList.length > 0 && (
              <>
                ...
                <div className="px-3 pt-1.5 pb-1">
                  <span className="text-[9px] font-medium text-on-surface/30 uppercase tracking-wider">Saved</span>
                </div>
                {savedList.map((item) => (
                  <button
                    key={item.id}
                    ...
                    className="w-full text-left px-3 py-2 hover:bg-white/5 transition-colors focus:outline-none focus:bg-white/5"
                  >
                    <div className="text-xs text-on-surface/70">{item.name}</div>
                  </button>
                ))}
              </>
            )}
      ...
      {/* Stats */}
      <div className="flex items-center gap-3 text-[10px] text-on-surface/30">
          ...
          {showLiveErrors && (
            <div className="absolute top-full right-0 mt-1 w-72 bg-surface-container border border-white/10 rounded-xl shadow-2xl z-50 p-3 max-h-48 overflow-y-auto">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[9px] font-medium text-on-surface/30 uppercase tracking-wider">Issues</span>
                <button onClick={() => setShowLiveErrors(false)} className="text-on-surface/30 hover:text-on-surface/60 text-xs" aria-label="Close issues panel">&times;</button>
              </div>
      ...
      {/* Pipeline name */}
      <span className="text-xs text-on-surface/40">{graph.name || 'Untitled Pipeline'}</span>
    </div>
  );
}
